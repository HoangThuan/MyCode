    ...
    fiterListItem() {
      this.listItems = this.listItemRoots.filter((item) => {
        if(this.sbWorkTime != null){
            ...
            item.workPerson.includes(this.sbWorkPerson) &&
            new Date(item.workTime).getFullYear() == new Date(this.sbWorkTime).getFullYear() &&
            new Date(item.workTime).getMonth() == new Date(this.sbWorkTime).getMonth() &&
            new Date(item.workTime).getDate() == new Date(this.sbWorkTime).getDate()
          );
        }
        else{
            return (
...
<template>
  <Layout>
    <PageHeader title="Internal Charging List" pageTitle="Merchant" />
        ...
        <BRow style="margin-top: 10px">
          <BCol lg="12">
            <div class="pay-table pay-table-title" style="text-align: center">
              <div class="with-16 pay-table-row no-boder-left">操作人员</div>
              <div class="with-14 pay-table-row no-boder-left">商户ID</div>
              <div class="with-15 pay-table-row no-boder-left">商户名称</div>
              <div class="with-14 pay-table-row no-boder-left">内充金额</div>
              <div class="with-14 pay-table-row no-boder-left">手续费</div>
              <div class="with-14 pay-table-row no-boder-left">时间</div>
            ...
            <div v-if="listItems.length > 0">
              <div
                ...
                v-bind:key="index"
              >
              <div class="with-16 pay-table-row no-boder-left">{{item.workPerson}}</div>
              <div class="with-14 pay-table-row no-boder-left">{{item.customeID}}</div>
              <div class="with-15 pay-table-row no-boder-left">{{item.customeName}}</div>
              <div class="with-14 pay-table-row no-boder-left">{{item.neiChong}}</div>
              <div class="with-14 pay-table-row no-boder-left">{{item.pay}}</div>
              <div class="with-14 pay-table-row no-boder-left">{{item.workTime}}</div>
